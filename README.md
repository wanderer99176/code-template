...
| 模板目录名 | 描述与适用场景 | 核心特性 |
| :--- | :--- | :--- |
| `wanderer99176-py-template-01-mvp` | **MVP版**: 用于快速原型、一次性脚本或个人实验。采用扁平布局，追求最快的启动速度。 | 单文件、快速启动 |
| `wanderer99176-py-template-02-small`| **小型工具版**: 用于开发可分发的CLI工具或简单的功能库。引入了`src`布局和`tests`目录。 | src布局、测试框架 |
| `wanderer99176-py-template-03-medium`| **中型应用版**: 用于构建标准的Web应用、API服务或有一定规模的库。增加了CI/CD和Docker支持。 | CI/CD、Docker |
| `wanderer99176-py-template-05-enterprise`| **企业级版**: 用于大型、多人协作、长期维护的复杂项目。包含了完整的分层架构和所有工程化组件。 | 完整架构、文档齐全 |
| `wanderer99176-py-template-06-project01`| **🔥 终极版 - 全栈AI应用Monorepo**: 企业级全栈AI应用，包含FastAPI后端、Nuxt前端、ML/Audio API、爬虫、完整DevOps和监控体系。 | Monorepo、微服务、AI集成、K8s |
...
#### **模板六：全栈AI应用Monorepo（终极版）🔥**

用于构建包含多个服务的大型全栈AI应用，支持微服务架构和云原生部署。

```bash
cookiecutter gh:wanderer99176/code-template --directory wanderer99176-py-template-06-project01
```
...
详细使用指南请查看：`wanderer99176-py-template-06-project01/使用指南.md`
...
| 特性 | MVP | Small | Medium | Enterprise | **Monorepo (终极版)** |
|:---|:---:|:---:|:---:|:---:|:---:|
...
1. **快速验证想法** → 使用模板一（MVP）
2. **开发CLI工具** → 使用模板二（Small）
3. **构建Web应用** → 使用模板三（Medium）
4. **大型企业项目** → 使用模板五（Enterprise）
5. **全栈AI微服务** → 使用模板六（Monorepo）🔥

### 模板升级路径

```
MVP → Small → Medium → Enterprise → Monorepo
```
